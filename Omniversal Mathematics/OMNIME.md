...
See [`Omniversal Calculator/CALCME.md`](./Omniversal%20Calculator/CALCME.md) for the full project details.

## Notation

The section's theoretical foundation is **Omnidirectional Mathematics** -- a formal notation system for describing transformations across dimensional spaces. Where conventional mathematics operates within a fixed dimensional framework, Omnidirectional Mathematics treats dimensionality itself as a variable.

The core axiom: **Movement = Transformation Sequence**. Every expression is a path:

```
α ⟿ [operation set] ⟿ ω
```

Twelve fundamental operations (dimensional ascension/descension, rotational transforms, polarity reversal, wave collapse/expansion, intersection marking, parallel/orthogonal modes, boundary crossing, infinite recursion, void traversal) compose into expressions that describe movement between any two points in any dimensional space.

See [`NOTATION.md`](./NOTATION.md) for the full formal specification with operator definitions, state semantics, examples, and properties.
