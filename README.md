...
- **[GeoVox](./Minecraft%20Innovations/GeoVox/)** (v0.2.0) -- a modular Python pipeline for importing real-world 3D data (heightmaps, with LiDAR/mesh/GeoJSON planned) into playable Minecraft worlds with palette-driven voxelization. Working pipeline: heightmap ingest, grid engine with slope computation, palette composition with elevation/slope conditions, 5 themed palettes, .mcfunction, .nbt structure, and .litematic schematic export. 48 passing tests.
- **[Minecraft Studio](./Minecraft%20Innovations/Minecraft%20Studio/)** (v0.3.0) -- a standalone IDE for Minecraft mod creation modeled on Roblox Studio. Layer 1 (data model + export engine) is fully complete: mod project model (blocks, items, entities, biomes, recipes, loot tables), Java code generation for entities and worldgen, and working exporters for Fabric, Forge, NeoForge, and vanilla Data Packs with entity registration, worldgen/biome export, language files, tag generation, creative tabs, and placeholder textures. 158 passing tests.
...
|-------|---------------|-------|----------|
| **GeoVox** | Unit + integration tests for the voxel pipeline -- grid, slope, palette, composer, heightmap ingest, three exporters, NBT writer | 48 | `make test-geovox` |
...
| **Minecraft Studio** | Unit + integration tests for the mod data model and export engine -- blocks, items, entities, biomes, recipes, loot tables, project serialization, Java codegen (entity + worldgen), Fabric/Forge/NeoForge/Data Pack export with entity registration, worldgen, lang files, tags, creative tabs, placeholder textures | 158 | `make test-mcstudio` |
...
Two projects. [`GeoVox`](./Minecraft%20Innovations/GeoVox/) (v0.2.0) is a working Python pipeline -- `pip install -e .` then `geovox heightmap.png out.mcfunction` (see [`VOXELME.md`](./Minecraft%20Innovations/GeoVox/VOXELME.md)). [`Minecraft Studio`](./Minecraft%20Innovations/Minecraft%20Studio/) (v0.3.0) -- `pip install -e .` then `mcstudio new my_mod && mcstudio add-block my_mod.json custom_ore && mcstudio export my_mod.json fabric` to create and export a mod project (see [`STUDYME.md`](./Minecraft%20Innovations/Minecraft%20Studio/STUDYME.md)). The theme overview is [`Minecraft Innovations/MCME.md`](./Minecraft%20Innovations/MCME.md).
