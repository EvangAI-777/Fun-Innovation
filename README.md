...
- **[AutoMuse](./Audio%20Innovations/AutoMuse/)** (v0.2.0, testing phase) -- a hybrid desktop DAW + AI chatbot shipping as `automuse.exe`, drawing from FL Studio/Ableton/Audacity for the DAW side and Claude/Gemini/ChatGPT for the conversational AI side. Full standalone DAW that works without AI *and* full "vibe-coding" style creation via conversation -- both modes simultaneously. Each module is developed and tested in the Python CLI, then compiled into the desktop application. Testing phase features: music theory engine (28 scales, 25 chord types, key signatures, rhythm), harmony (10 progression templates, 6 voicing styles, harmonic analysis), composition (melody generation with 6 contour shapes, motif system with classical transformations, arrangement model with 5 song structure templates), export (MIDI + MusicXML), and the Muse conversational engine (21 commands, session save/load). 310 passing tests.
...
| **AutoMuse** | Unit + integration tests for the composition engine -- notes, intervals, scales, chords, keys, rhythm, progressions, harmonic analysis, voicings, motifs, melody generation, arrangement, MIDI writer, MusicXML export, and the Muse conversational interface | 310 | `make test-automuse` |
...
New approaches to music and sound production. Current project: [`AutoMuse`](./Audio%20Innovations/AutoMuse/) (v0.2.0, testing phase) -- `pip install -e .` then `automuse` or `python -m automuse` to start a session with the Muse (see [`MUSEME.md`](./Audio%20Innovations/AutoMuse/MUSEME.md)). The theme overview is [`Audio Innovations/HEARME.md`](./Audio%20Innovations/HEARME.md).
