...
- **The user handles the final push** — At the end of each working session, the user will push everything to the branch from their side. Claude's job is to commit frequently so that no work is lost and progress is always saved to the local git history.
